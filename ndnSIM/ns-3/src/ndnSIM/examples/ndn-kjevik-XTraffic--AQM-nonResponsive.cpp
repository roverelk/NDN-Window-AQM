/** 
 * ndn-kjevik-XTraffic--AQM-nonResponsive.cpp
 * 
 ...
 *     NS_LOG=ndn.Consumer:ndn.Producer ./waf --run=ndn-kjevik-XTraffic--AQM-nonResponsive
 *
...
main(int argc, char* argv[])
{
  ...
  std::string m_outputName = "ndn-kjevik-XTraffic--v3-2--Exponential-AQM-PCON-AQMatR2";
  double m_granularity = 1; // time in seconds

  std::string prefix[2] = {"/k/A", "/k/B"};

  // Choose where to place AQM
  bool R1 = false; // write true or false
  bool R2 = true;
  ...
  if(R1 == true){
    ndn::StrategyChoiceHelper::Install(router_R1[0], "/", "/localhost/nfd/strategy/multicast-Eirik-v2-R1");
  }else{
    ndn::StrategyChoiceHelper::Install(router_R1[0], "/", "/localhost/nfd/strategy/best-route");
  }
  if(R2 == true){
    ndn::StrategyChoiceHelper::Install(router_R2[0], "/", "/localhost/nfd/strategy/multicast-Eirik-v2-R2");
  }else{
