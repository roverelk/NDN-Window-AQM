...
main(int argc, char* argv[])
{
  ...
  std::string export_file_names = "master-wifi-test";
  std::string export_file_version = "v2";
  double v_recordGranularity = 0.25; // seconds
  double v_simulationTime = 240; // seconds
  ...
  wifiPhyHelper.Set("TxPowerStart", DoubleValue(5)); // dBm
  wifiPhyHelper.Set("TxPowerEnd", DoubleValue(5)); // dBm
  ...
  std::string topoFile = "src/ndnSIM/examples/topologies/topo-kjevik-5C-R-P-wifi.txt";
  ...
  consumers.Add(Names::Find<Node>("C5"));
  // NodeContainer routers;
  consumers.Add(Names::Find<Node>("R1"));

  ////////////////
  // 1. Install Wifi
  // NetDeviceContainer wifiNetDevices = wifi.Install(wifiPhyHelper, wifiMacHelper, routers);
  NetDeviceContainer wifiNetDevices = wifi.Install(wifiPhyHelper, wifiMacHelper, consumers);
  // NetDeviceContainer wifiNetDevices = wifi.Install(wifiPhyHelper, wifiMacHelper, routers[1]);
  ...
  for(int i = 0; i < 5; i++)
  {
    ndn::StrategyChoiceHelper::Install(consumers[i], "/", "/localhost/nfd/strategy/multicast-Eirik  ");
  }
  // ndn::StrategyChoiceHelper::Install(routers[0], "/", "/localhost/nfd/strategy/multicast");
  ...
  std::string prefix[7] = {"/k/A", "/k/B", "/k/C", "/k/D", "/k/E", "/k/F", "/k/G"};
  ...
  consumerHelper.SetAttribute("CcAlgorithm", StringValue("CUBIC")); // AIMD, BIC or CUBIC

  for(int i = 0; i < 5; i++)
  {
    consumerHelper.SetPrefix(prefix[i]);
    consumerHelper.SetAttribute("StartTime", StringValue("1"));
  ...
  producerHelper.SetAttribute("PayloadSize", StringValue("60"));
