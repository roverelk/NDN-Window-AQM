...
main(int argc, char* argv[])
{
  ...
  std::string m_outputName = "STraffic-PCONvCBR200-Benchmark-A1024B1024-Seed1-Run" + std::to_string(runs) + "-Start" + std::to_string(start_time);
  ...
  if(R1){
    ndn::StrategyChoiceHelper::Install(router_R1[0], "/", "/localhost/nfd/strategy/multicast-STraffic-v1-R1");
  }else{
    ndn::StrategyChoiceHelper::Install(router_R1[0], "/", "/localhost/nfd/strategy/best-route");
  }
  if(R2){
    ndn::StrategyChoiceHelper::Install(router_R2[0], "/", "/localhost/nfd/strategy/multicast-STraffic-v1-R2");
  }else{
