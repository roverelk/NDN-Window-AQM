...
#include "multicast-strategy-BTraffic-v1-R1.hpp"
...
NFD_REGISTER_STRATEGY(MulticastStrategyBTraffic1_R1);

NFD_LOG_INIT("MulticastStrategyBTraffic1_R1");

const time::milliseconds MulticastStrategyBTraffic1_R1::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds MulticastStrategyBTraffic1_R1::RETX_SUPPRESSION_MAX(250);



void
MulticastStrategyBTraffic1_R1::updateTime ()
{
...
void
MulticastStrategyBTraffic1_R1::updateShaper ()
{
  ...
  if(downStreamInInterest_A_Quantity > 1 && downStreamInInterest_B_Quantity > 1){
    numberOfIntToSend_A = ((BANDWIDTH / 2) / (avgDataSize_A + 28.0));
    numberOfIntToSend_B = ((BANDWIDTH / 2) / (avgDataSize_B + 28.0));
  ...
  if(downStreamInData_C_Quantity > 1){
    avgDataSize_C = downStreamInData_C_Size / downStreamInData_C_Quantity;
  }
  numberOfDataToSend_C = (BANDWIDTH - ((numberOfIntToSend_A * 28) + (numberOfIntToSend_B*28))) / avgDataSize_C;
...
MulticastStrategyBTraffic1_R1::MulticastStrategyBTraffic1_R1(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
...
const Name&
MulticastStrategyBTraffic1_R1::getStrategyName()
{
  static Name strategyName("/localhost/nfd/strategy/multicast-BTraffic-v1-R1/%FD%01");
  return strategyName;
}

void
MulticastStrategyBTraffic1_R1::afterReceiveInterest(const Face& inFace, const Interest& interest,
                                        const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  if((inFace.getId() == 256) || inFace.getId() == 257 || inFace.getId() == 258){
...
void
MulticastStrategyBTraffic1_R1::afterReceiveNack(const Face& inFace, const lp::Nack& nack,
                                    const shared_ptr<pit::Entry>& pitEntry)
{
...
void
MulticastStrategyBTraffic1_R1::afterReceiveData(const shared_ptr<pit::Entry>& pitEntry,
