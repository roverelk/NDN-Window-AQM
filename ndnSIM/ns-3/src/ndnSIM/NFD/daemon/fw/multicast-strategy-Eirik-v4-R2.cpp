...
#include "multicast-strategy-Eirik-v4-R2.hpp"
...
NFD_REGISTER_STRATEGY(MulticastStrategyEirik4_R2);

NFD_LOG_INIT("MulticastStrategyEirik4_R2");

const time::milliseconds MulticastStrategyEirik4_R2::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds MulticastStrategyEirik4_R2::RETX_SUPPRESSION_MAX(250);



void
MulticastStrategyEirik4_R2::updateTime ()
{
...
void
MulticastStrategyEirik4_R2::updateBandwidth ()
{
  float avgDataSize = downStreamInDataSize / downStreamInDataQuantity;
  
  //std::cout << "upStreamInDataQuantity:\t" << upStreamInDataQuantity << "\tdownStreamInDataQuantity:\t" << downStreamInDataQuantity << "\n";

  if(upStreamInDataQuantity > 5){
    ...
    float numberOfInterestToFillDownStream = BANDWIDTH / ((upStreamInDataSize / upStreamInDataQuantity) + 20.0);
    //std::cout << "numberOfInterestToFillDownStream:\t" << numberOfInterestToFillDownStream << "\n";
    ...
    numberOfDataToSend = (BANDWIDTH - (numberOfInterestToSend*25)) / avgDataSize; 

    //std::cout << "U\tInterest:\t" << numberOfInterestToSend << "\tData:\t" << numberOfDataToSend << "\n";
...
MulticastStrategyEirik4_R2::MulticastStrategyEirik4_R2(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
...
const Name&
MulticastStrategyEirik4_R2::getStrategyName()
{
  static Name strategyName("/localhost/nfd/strategy/multicast-Eirik-v4-R2/%FD%04");
  return strategyName;
}

void
MulticastStrategyEirik4_R2::afterReceiveInterest(const Face& inFace, const Interest& interest,
...
void
MulticastStrategyEirik4_R2::afterReceiveNack(const Face& inFace, const lp::Nack& nack,
                                    const shared_ptr<pit::Entry>& pitEntry)
{
...
void
MulticastStrategyEirik4_R2::afterReceiveData(const shared_ptr<pit::Entry>& pitEntry,
