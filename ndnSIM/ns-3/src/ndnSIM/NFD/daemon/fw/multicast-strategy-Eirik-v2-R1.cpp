...
#include "multicast-strategy-Eirik-v2-R1.hpp"
...
NFD_REGISTER_STRATEGY(MulticastStrategyEirik2_R1);

NFD_LOG_INIT("MulticastStrategyEirik2_R1");

const time::milliseconds MulticastStrategyEirik2_R1::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds MulticastStrategyEirik2_R1::RETX_SUPPRESSION_MAX(250);

void
MulticastStrategyEirik2_R1::updateTime ()
{
...
void
MulticastStrategyEirik2_R1::updateBandwidth ()
{
...
MulticastStrategyEirik2_R1::MulticastStrategyEirik2_R1(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
...
const Name&
MulticastStrategyEirik2_R1::getStrategyName()
{
  static Name strategyName("/localhost/nfd/strategy/multicast-Eirik-v2-R1/%FD%02");
  return strategyName;
}

void
MulticastStrategyEirik2_R1::afterReceiveInterest(const Face& inFace, const Interest& interest,
                                        const shared_ptr<pit::Entry>& pitEntry)
{
  bool doNormalStuff = false;
  // inInterest from Consumer
  if(inFace.getId() == 256){
    this->updateTime();
...
void
MulticastStrategyEirik2_R1::afterReceiveNack(const Face& inFace, const lp::Nack& nack,
                                    const shared_ptr<pit::Entry>& pitEntry)
{
...
void
MulticastStrategyEirik2_R1::afterReceiveData(const shared_ptr<pit::Entry>& pitEntry,
                           const Face& inFace, const Data& data)
{
  bool doNormalStuff = false;
  //inData from Producer
  if(inFace.getId() == 257){
