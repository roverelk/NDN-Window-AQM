...
#include "multicast-strategy-BTraffic-v1-R2.hpp"
...
NFD_REGISTER_STRATEGY(MulticastStrategyBTraffic1_R2);

NFD_LOG_INIT("MulticastStrategyBTraffic1_R2");

const time::milliseconds MulticastStrategyBTraffic1_R2::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds MulticastStrategyBTraffic1_R2::RETX_SUPPRESSION_MAX(250);



void
MulticastStrategyBTraffic1_R2::updateTime ()
{
...
void
MulticastStrategyBTraffic1_R2::updateShaper ()
{
  float avgDataSize_A = 1024;
  float avgDataSize_B = 1024;
  float avgDataSize_C = 1024;
  ...
  if(upStreamInData_C_Quantity > 1){
    avgDataSize_C = upStreamInData_C_Size / upStreamInData_C_Quantity;
  }else if(downStreamInData_C_Quantity > 1){
    avgDataSize_C = downStreamInData_C_Size / downStreamInData_C_Quantity;
  }

  // If not received Data packets from upstream
  // Received Interest packets from downstream
  if(downStreamInInterest_A_Quantity > 1 && downStreamInInterest_B_Quantity > 1 && downStreamInInterest_C_Quantity > 1){
    numberOfIntToSend_A = (BANDWIDTH / 3) / (avgDataSize_A + 28.0);
    numberOfIntToSend_B = (BANDWIDTH / 3) / (avgDataSize_B + 28.0);
    numberOfIntToSend_C = (BANDWIDTH / 3) / (avgDataSize_C + 28.0);
    ...
    if(downStreamInInterest_A_Quantity > 1){
      ...
      numberOfIntToSend_C = numberOfIntToSend_A;
      numberOfDataToSend_C = numberOfDataToSend_A;
    ...
    // Only received Interest from B
      ...
      numberOfIntToSend_C = numberOfIntToSend_B;
      numberOfDataToSend_C = numberOfDataToSend_B;

    // Only received Interest from C
    }else if(downStreamInInterest_C_Quantity > 1){
      numberOfIntToSend_C = BANDWIDTH / (avgDataSize_C + 28.0);
      numberOfDataToSend_C = (BANDWIDTH - (numberOfIntToSend_C*28)) / avgDataSize_C;
      
      // To allow any other namespaces to also be allowed to pass through
      numberOfIntToSend_A = numberOfIntToSend_C;
      numberOfDataToSend_A = numberOfDataToSend_C/2;
      numberOfIntToSend_B = numberOfIntToSend_C;
      numberOfDataToSend_B = numberOfDataToSend_C/2;;
      ...
      numberOfIntToSend_C = BANDWIDTH / (1024.0 + 28.0);
      ...
      if(downStreamInData_A_Quantity > 1 && downStreamInData_B_Quantity > 1){
        ...
        numberOfDataToSend_C = (BANDWIDTH - (numberOfIntToSend_C*28)) / 1024;
      }
    }
  }
  ...
  downStreamInInterest_C_Quantity = 1.0;
  downStreamInInterest_C_Size = 1.0;
  downStreamInData_C_Quantity = 1.0;
  downStreamInData_C_Size = 1.0;
  ...
  upStreamInInterest_C_Quantity = 1.0;
  upStreamInInterest_C_Size = 1.0;
  upStreamInData_C_Quantity = 1.0;
  upStreamInData_C_Size = 1.0;

  timeSinceLastCalc = 0;

}

MulticastStrategyBTraffic1_R2::MulticastStrategyBTraffic1_R2(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
...
const Name&
MulticastStrategyBTraffic1_R2::getStrategyName()
{
  static Name strategyName("/localhost/nfd/strategy/multicast-BTraffic-v1-R2/%FD%01");
  return strategyName;
}

void
MulticastStrategyBTraffic1_R2::afterReceiveInterest(const Face& inFace, const Interest& interest,
                                        const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  if((inFace.getId() == 257) || inFace.getId() == 258 || inFace.getId() == 259){
    ...
    else if((name.find(prefixC) != std::string::npos) && (numberOfIntToSend_C >= 0)){
      numberOfIntToSend_C--;
      downStreamInInterest_C_Quantity++;
      downStreamInInterest_C_Size += 28;
      doNormalStuff = true;
    }
    ...
    if(name.find(prefixA) != std::string::npos){
      ...
      upStreamInInterest_A_Size += 28;
    }
    else if(name.find(prefixB) != std::string::npos){
      upStreamInInterest_B_Quantity++;
      upStreamInInterest_B_Size += 28;
    }
    else if(name.find(prefixC) != std::string::npos){
      upStreamInInterest_C_Quantity++;
      upStreamInInterest_C_Size += 28;
    }
    doNormalStuff = true;
...
void
MulticastStrategyBTraffic1_R2::afterReceiveNack(const Face& inFace, const lp::Nack& nack,
                                    const shared_ptr<pit::Entry>& pitEntry)
{
...
void
MulticastStrategyBTraffic1_R2::afterReceiveData(const shared_ptr<pit::Entry>& pitEntry,
                           const Face& inFace, const Data& data)
{
  ...
  if(inFace.getId() == 257 || inFace.getId() == 258){
    ...
    else if((name.find(prefixC) != std::string::npos) && (numberOfDataToSend_C >= 0)){
      numberOfDataToSend_C--;
      downStreamInData_C_Quantity++;
      downStreamInData_C_Size += data.getContent().size();
      doNormalStuff = true;
    }
    ...
    if(name.find(prefixA) != std::string::npos){
      ...
      upStreamInData_C_Quantity++;
      upStreamInData_C_Size += data.getContent().size();
    }
    doNormalStuff = true;
